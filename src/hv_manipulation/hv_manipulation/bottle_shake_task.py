...
class BottleShakeTask(Node):
    ...
    def __init__(self):
        ...
        # Number of complete strokes in EACH stage: first vertical,
        # then horizontal.
        self.declare_parameter("shake_cycles", 3)
        self.declare_parameter("shake_samples_per_cycle", 32)
        self.declare_parameter("shake_wrist_rotation_degrees", 45.0)
        # 1.0 means the maximum velocity/acceleration allowed by the
        # Panda joint limits configured in MoveIt.
        self.declare_parameter("shake_velocity", 1.0)
        self.declare_parameter("shake_acceleration", 1.0)
        # Gazebo-only timing compression. 0.55 executes the same path in
        # 55% of MoveIt's planned time (about 1.82x faster).
        self.declare_parameter("shake_time_scale", 0.55)
        # A 1 cm Cartesian interpolation step is sufficiently smooth for
        # this open-air shake and plans much faster than the old 2.5 mm.
        self.declare_parameter("shake_cartesian_max_step", 0.01)
        # Briefly hold the exact final joint target so the fast-moving
        # wrist converges before MoveIt plans the placement trajectory.
        self.declare_parameter("shake_endpoint_hold_seconds", 0.45)
        ...
        self.shake_wrist_rotation_degrees = float(
            self.get_parameter(
                "shake_wrist_rotation_degrees"
            ).value
        )
        ...
        self.shake_time_scale = float(
            self.get_parameter("shake_time_scale").value
        )
        self.shake_cartesian_max_step = float(
            self.get_parameter("shake_cartesian_max_step").value
        )
        self.shake_endpoint_hold_seconds = float(
            self.get_parameter("shake_endpoint_hold_seconds").value
        )
    ...
    def normalize_quaternion(
        ...
        """Return a unit quaternion in MoveIt XYZW ordering."""
        ...
        return [component / norm for component in quaternion_xyzw]
    ...
    def multiply_quaternions(
        ...
        """Compose two quaternions, returning left * right in XYZW."""

        left_x, left_y, left_z, left_w = left_xyzw
        right_x, right_y, right_z, right_w = right_xyzw
    ...
    def wrist_rock_orientation(
        self,
        base_orientation_xyzw: Sequence[float],
        angle_radians: float,
    ) -> list[float]:
        """Rock the wrist about the gripper's local Z connection axis."""
        ...
        composed = self.multiply_quaternions(
            base_orientation_xyzw,
            local_z_rotation,
        )
        return self.normalize_quaternion(composed)

    def build_long_fast_shake_waypoints(
        self,
        ...
        """Build long vertical strokes, then long horizontal strokes.

        Wrist rocking is continuous in both stages. It runs at twice the
        translation frequency so the wrist is still moving when the arm
        reverses at the end of each stroke. This avoids an instantaneous
        180-degree path cusp while preserving a visibly straight up/down
        or side-to-side positional motion.
        """

        waypoints: list[Pose] = []
        total_samples = cycles * samples_per_cycle
        base_orientation = self.normalize_quaternion(
            orientation_xyzw
        )
        wrist_amplitude = math.radians(
            self.shake_wrist_rotation_degrees
        )

        def append_stage(axis: str, amplitude: float) -> None:
            for sample_number in range(1, total_samples + 1):
                ...
                pose = Pose()
                pose.position.x = center_position[0]
                pose.position.y = center_position[1]
                pose.position.z = center_position[2]

                translation = amplitude * math.sin(phase)

                if axis == "vertical":
                    pose.position.z += translation
                else:
                    pose.position.x += translation

                # Two wrist rocks per translational cycle. At each
                # positional reversal, the wrist keeps rotating instead
                # of asking the time parameterizer to make a hard cusp.
                wrist_angle = wrist_amplitude * math.sin(
                    2.0 * phase
                )
                orientation = self.wrist_rock_orientation(
                    base_orientation,
                    wrist_angle,
                )
        ...
        # The order is intentional and matches the requested action:
        # lengthy vertical first, then lengthy horizontal.
        append_stage("vertical", vertical_amplitude)
        append_stage("horizontal", horizontal_amplitude)
    ...
    def execute_continuous_mixing_path(
        ...
        # Let the final post-lift joint-state message reach MoveIt before
        # requesting the long shake. Do not copy pymoveit2's cached state
        # into request.start_state: it can lag the controller and caused
        # the earlier "start point deviates" execution abort. An empty
        # start_state makes move_group snapshot its own current state.
        ...
        # The shake happens in open space. A larger interpolation step
        # avoids thousands of unnecessary IK samples and removes most of
        # the delay between lifting and beginning the shake.
        request.max_step = self.shake_cartesian_max_step
        ...
        self.get_logger().info(
            f"[PLAN] LONG FAST VERTICAL + HORIZONTAL SHAKE: "
        ...
        original_duration_seconds = trajectory_times[-1]

        if original_duration_seconds <= 0.0:
            self.get_logger().error(
                "[MIX FAILED] Trajectory was not time-parameterized"
            )
            return False

        # MoveIt scaling factors stop at 1.0. For this Gazebo shake,
        # compress every timestamp after planning while scaling the
        # optional velocity and acceleration fields consistently. The
        # Cartesian path itself remains exactly unchanged.
        for point in trajectory.points:
            original_time = (
                point.time_from_start.sec
                + point.time_from_start.nanosec * 1e-9
            ...
            point.time_from_start = Duration(
                seconds=original_time * self.shake_time_scale
            ).to_msg()

            if len(point.velocities) > 0:
                point.velocities = [
                    velocity / self.shake_time_scale
            ...
            if len(point.accelerations) > 0:
                point.accelerations = [
                    acceleration
                    / (self.shake_time_scale ** 2)
                    for acceleration in point.accelerations
                ]

        # Add a stationary endpoint after the compressed motion. This
        # gives panda_joint7 time to converge exactly to the base grasp
        # orientation instead of immediately planning from a small wrist
        # tracking error.
        ...
        hold_point.positions = list(final_point.positions)

        if len(final_point.velocities) > 0:
            hold_point.velocities = [
                0.0 for _ in final_point.velocities
            ]

        if len(final_point.accelerations) > 0:
            hold_point.accelerations = [
                0.0 for _ in final_point.accelerations
            ]

        if len(final_point.effort) > 0:
            hold_point.effort = list(final_point.effort)

        hold_point.time_from_start = Duration(
            seconds=(
                final_motion_time
                + self.shake_endpoint_hold_seconds
            )
        ).to_msg()
        ...
        for index in range(1, len(trajectory_times)):
            if trajectory_times[index] <= trajectory_times[index - 1]:
                self.get_logger().error(
                    "[MIX FAILED] Faster trajectory timestamps are "
                    "not strictly increasing"
                )
                return False
        ...
        average_frequency = total_cycles / duration_seconds
        speedup = 1.0 / self.shake_time_scale

        self.get_logger().info(
            f"[PLANNED] LONG FAST SHAKE: "
            f"fraction={response.fraction:.3f} | "
            f"trajectory_points={len(trajectory.points)} | "
            f"MoveIt_duration={original_duration_seconds:.2f}s | "
            f"fast_duration={duration_seconds:.2f}s | "
            f"speedup={speedup:.2f}x | "
            f"overall_cycle_rate={average_frequency:.2f}Hz"
        )

        self.get_logger().info(
            "[EXECUTE] VERTICAL FIRST, THEN HORIZONTAL: one trajectory"
        )
        ...
        # Process the controller's final joint-state updates before the
        # next pymoveit2 plan. Without this synchronization, placement can
        # be planned from the pre-settled wrist state and fail MoveIt's
        # 0.01-radian start-tolerance check.
        state_sync_deadline = time.monotonic() + 0.30

        while rclpy.ok() and time.monotonic() < state_sync_deadline:
            rclpy.spin_once(self, timeout_sec=0.02)

        self.get_logger().info(
            "[SUCCESS] LONG FAST VERTICAL + HORIZONTAL SHAKE"
        )
        return True
    ...
    def perform_mixing_shake(
        ...
        """Run vertical strokes, then horizontal strokes, with wrist rock."""
        ...
        if not 0.25 <= self.shake_time_scale <= 1.0:
            self.get_logger().error(
                "[SHAKE FAILED] shake_time_scale must be in [0.25, 1.0]"
            )
            return False

        if not 0.005 <= self.shake_cartesian_max_step <= 0.02:
            self.get_logger().error(
                "[SHAKE FAILED] shake_cartesian_max_step must be "
                "in [0.005, 0.02] metres"
            )
            return False

        if not 0.20 <= self.shake_endpoint_hold_seconds <= 1.0:
            self.get_logger().error(
                "[SHAKE FAILED] shake_endpoint_hold_seconds must be "
                "in [0.20, 1.0] seconds"
            )
            return False

        if not 0.0 < self.shake_wrist_rotation_degrees <= 60.0:
            self.get_logger().error(
                "[SHAKE FAILED] Wrist rotation must be in (0, 60] degrees"
            )
            return False

        try:
            orientation_xyzw = self.normalize_quaternion(
                orientation_xyzw
            )
        except ValueError as error:
            self.get_logger().error(
                f"[SHAKE FAILED] Invalid orientation: {error}"
            )
            return False

        waypoints = self.build_long_fast_shake_waypoints(
        ...
        self.get_logger().info(
            f"[LONG FAST SHAKE] vertical={cycles} cycles first | "
            f"horizontal={cycles} cycles second | "
            f"samples_per_cycle={self.shake_samples_per_cycle} | "
            f"horizontal=±{horizontal_amplitude:.3f}m | "
            f"vertical=±{vertical_amplitude:.3f}m | "
            f"wrist_rock=±{self.shake_wrist_rotation_degrees:.1f}deg | "
            f"velocity={self.shake_velocity:.2f} | "
            f"acceleration={self.shake_acceleration:.2f} | "
            f"simulation_speedup={1.0 / self.shake_time_scale:.2f}x | "
            f"Cartesian_step={self.shake_cartesian_max_step:.3f}m | "
            f"endpoint_hold={self.shake_endpoint_hold_seconds:.2f}s"
        )
        ...
        self.get_logger().info(
            "[SHAKE COMPLETE] Bottle returned to lift centre"
        )
        return True
    # ==============================================================
